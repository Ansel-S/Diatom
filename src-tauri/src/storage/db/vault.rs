...
use super::core::Db;
use super::types::{VaultLoginRaw, VaultCardRaw, VaultNoteRaw};
...
impl Db {
    ...
    pub fn vault_login_upsert(&self, r: &VaultLoginRaw) -> Result<()> {
        self.0.lock().unwrap().execute(
            ...
            params![
                r.id, r.title, r.username, r.password_enc, r.urls_json,
                r.notes_enc, r.tags_json, r.totp_uri,
                r.breach_status, r.breach_checked_at, r.breach_pwned_count,
                r.passkey_cred_id_enc, r.passkey_user_handle, r.passkey_rp_id, r.passkey_added_at,
    ...
    pub fn vault_logins_raw(&self) -> Result<Vec<VaultLoginRaw>> {
        ...
        let rows = stmt.query_map([], |r| Ok(VaultLoginRaw {
            ...
            tags_json:           r.get(6)?,
            totp_uri:            r.get(7)?,
            breach_status:       r.get::<_, Option<String>>(8)?.unwrap_or_else(|| "unknown".into()),
    ...
    pub fn breach_cache_evict_old(&self, max_age_secs: i64) -> Result<usize> {
        ...
        let n = self.0.lock().unwrap().execute(
            "DELETE FROM vault_breach_cache WHERE fetched_at < ?1", [cutoff])?;
        Ok(n)
    }
