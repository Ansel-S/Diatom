// ─────────────────────────────────────────────────────────────────────────────
// diatom/src-tauri/src/net_monitor.rs
//
// Outgoing Connection Monitor — "Proof of Privacy" Outbound Traffic Audit
//
// Core promise: Diatom claims "zero data exfiltration". This module lets users
// verify that claim: every socket connection initiated by the Diatom process
// is recorded and displayed in real time in the UI.
// Users can distinguish "web content requests" (user behaviour) from
// "Diatom-own requests" (framework behaviour).
//
// Implementation:
//   - All outbound requests from the process must call AppState::net_monitor.record()
//   - Tracked: Sentinel version queries, threat list updates, filter list fetches,
//              Nostr relay connections.
//   - Not tracked: web content loaded by the user (that is user behaviour).
//   - Frontend: cmd_net_monitor_log  → fetches the last 500 logged entries
//   - Frontend: cmd_net_monitor_clear → clears the log
// ─────────────────────────────────────────────────────────────────────────────
...
impl Default for NetMonitor {
    fn default() -> Self {
        Self { log: Mutex::new(VecDeque::with_capacity(MAX_LOG)) }
    }
}
...
impl NetMonitor {
    ...
    pub fn summary(&self) -> NetMonitorSummary {
        let log = self.log.lock().unwrap();
        let total = log.len();
        let diatom_own = log.iter().filter(|e| !matches!(e.purpose, NetPurpose::PageContent)).count();
        let unique_hosts: std::collections::HashSet<&str> =
            log.iter().filter(|e| !matches!(e.purpose, NetPurpose::PageContent))
               .map(|e| e.host.as_str()).collect();
        NetMonitorSummary {
            total_events: total,
...
fn redact_url(url: &str) -> String {
 // scheme + host + first 32 chars of path; query/fragment are excluded.
    match url::Url::parse(url) {
        Ok(u) => {
            let path: String = u.path().chars().take(32).collect();
            let ellipsis = if u.path().len() > 32 { "…" } else { "" };
            format!("{}://{}{}{}", u.scheme(), u.host_str().unwrap_or("?"), path, ellipsis)
        }
...
mod tests {
    ...
    fn record_and_retrieve() {
        ...
        m.record(NetPurpose::SentinelVersionCheck, "https://versionhistory.googleapis.com/v1/chrome", Some(200), Some(1024));
    ...
    fn summary_counts_correctly() {
        let m = NetMonitor::default();
        m.record(NetPurpose::SentinelVersionCheck, "https://example.com/sentinel", Some(200), None);
        m.record(NetPurpose::PageContent, "https://news.ycombinator.com/", Some(200), None);
        let s = m.summary();
