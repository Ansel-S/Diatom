// ─────────────────────────────────────────────────────────────────────────────
// diatom/src-tauri/src/vault.rs  — v0.9.5
//
// Integrated password manager + TOTP authenticator vault.
// Inspired by Proton Pass architecture: every secret encrypted with AES-256-GCM
// using the existing Diatom master key (no separate account, no cloud).
//
// Features:
//   • Login entries: title, username, password, URL(s), notes, tags
//   • Credit card entries: cardholder, number, expiry, CVV, notes
//   • Secure note entries: title, content
//   • Password generator: random (configurable) and passphrase modes
//   • Domain autofill matching (same leading-dot logic as totp.rs)
//   • Import from CSV (Bitwarden, 1Password, LastPass common columns)
//   • Export to Diatom JSON (AES-encrypted passphrase backup)
//   • All sensitive fields encrypted individually → partial DB leak is safe
//
// Encryption: same AES-256-GCM + random 12-byte nonce as totp.rs.
// The master key is derived from the OS keychain at startup (see freeze.rs).
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Public summary types (safe to send to frontend, no raw secrets) ───────────
...
impl Default for PasswordConfig {
    fn default() -> Self {
        Self { length: 20, uppercase: true, numbers: true, symbols: true }
    }
}
...
pub fn generate_password(cfg: &PasswordConfig) -> String {
    ...
    // Ensure at least one char from each requested category
...
impl VaultStore {
    ...
    pub fn load_from_db(db: &crate::db::Db, key: &[u8; 32]) -> Self {
    ...
    fn decrypt_login(raw: &crate::db::VaultLoginRaw, key: &[u8; 32]) -> Result<VaultLogin> {
    ...
    fn decrypt_card(raw: &crate::db::VaultCardRaw, key: &[u8; 32]) -> Result<VaultCard> {
    ...
    fn decrypt_note(raw: &crate::db::VaultNoteRaw, key: &[u8; 32]) -> Result<VaultNote> {
    ...
    pub fn add_login(
        ...
        let raw = crate::db::VaultLoginRaw {
            id: id.clone(), title: title.to_owned(), username: username.to_owned(),
            password_enc, notes_enc, urls_json, tags_json,
            totp_uri: totp_uri.clone(), created_at: now, updated_at: now,
        };
        db.vault_login_upsert(&raw)?;

        let login = VaultLogin {
            id: id.clone(), title: title.to_owned(), username: username.to_owned(),
            password: password.to_owned(), urls: urls.clone(), notes: notes.to_owned(),
            tags: tags.clone(), totp_uri, created_at: now, updated_at: now,
        };
    ...
    pub fn update_login(
        ...
        let raw = crate::db::VaultLoginRaw {
            id: id.to_owned(), title: new_title.clone(), username: new_username.clone(),
            password_enc, notes_enc, urls_json, tags_json,
            totp_uri: new_totp_uri.clone(), created_at: entry.created_at, updated_at: now,
        };
        db.vault_login_upsert(&raw)?;

        let updated = VaultLogin {
            id: id.to_owned(), title: new_title, username: new_username,
            password: new_password, urls: new_urls, notes: new_notes,
            tags: new_tags, totp_uri: new_totp_uri, created_at: entry.created_at,
            updated_at: now,
        };
    ...
    pub fn delete_login(&mut self, id: &str, db: &crate::db::Db) -> Result<()> {
    ...
    pub fn add_card(
        ...
        let card = VaultCard {
            id: id.clone(), title: title.to_owned(), cardholder: cardholder.to_owned(),
            number: number.to_owned(), expiry: expiry.to_owned(), cvv: cvv.to_owned(),
            notes: notes.to_owned(), tags: tags.clone(), created_at: now, updated_at: now,
        };
    ...
    pub fn delete_card(&mut self, id: &str, db: &crate::db::Db) -> Result<()> {
    ...
    fn card_summary(c: &VaultCard) -> CardSummary {
        ...
        CardSummary {
            id: c.id.clone(), title: c.title.clone(), cardholder: c.cardholder.clone(),
    ...
    pub fn add_note(
        ...
        let note = VaultNote {
            id: id.clone(), title: title.to_owned(), content: content.to_owned(),
            tags: tags.clone(), created_at: now, updated_at: now,
        };
    ...
    pub fn delete_note(&mut self, id: &str, db: &crate::db::Db) -> Result<()> {
    ...
    fn note_summary(n: &VaultNote) -> NoteSummary {
        NoteSummary {
            id: n.id.clone(), title: n.title.clone(), tags: n.tags.clone(),
            created_at: n.created_at, updated_at: n.updated_at,
        }
    }

    // ── CSV import (Bitwarden / 1Password / LastPass common format) ───────────
...
mod tests {
    ...
    fn generate_password_respects_length() {
        let cfg = PasswordConfig { length: 24, uppercase: true, numbers: true, symbols: true };
    ...
    fn generate_passphrase_word_count() {
        let cfg = PassphraseConfig {
            word_count: 4, separator: "-".to_owned(), capitalise: false, include_number: false,
        };
