...
impl AppState {
    pub fn new(data_dir: PathBuf) -> Result<Self> {
        ...
        let storage_budget: crate::storage_guard::StorageBudget = db.get_setting("storage_budget")
            .and_then(|j| serde_json::from_str(&j).ok()).unwrap_or_default();

        let tab_budget_cfg: TabBudgetConfig = db.get_setting("tab_budget_config")
            .and_then(|j| serde_json::from_str(&j).ok()).unwrap_or_default();
    ...
    pub fn workspace_id(&self) -> String { self.ws_id.lock().unwrap().clone() }
    ...
    pub fn master_key(&self) -> [u8; 32] { *self.master_key.lock().unwrap() }
}
