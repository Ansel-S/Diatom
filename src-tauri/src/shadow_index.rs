// ─────────────────────────────────────────────────────────────────────────────
// diatom/src-tauri/src/shadow_index.rs  — v0.10.0
//
// Shadow Index — Decentralised Human Search Engine + Bias Contrast View
//
// Core idea: Search engines are overwhelmed by SEO and AI-generated spam.
// Shadow Index surfaces "authentic human footprints" instead of algorithmic
// ranking — every index entry originates from a real user who intentionally
// saved a page to their Museum.
//
// Two operating modes:
//
//   1. LOCAL MODE (default):
//      Searches the local Museum using TF-IDF full-text ranking.
//      No network requests. Fully offline.
//
//   2. P2P MODE (requires user opt-in):
//      Broadcasts a keyword hash (BLAKE3(keyword + salt)) to online Diatom
//      nodes via Nostr. Plaintext keywords are never transmitted.
//      Collects "anonymous index entries" from peer nodes and ranks them
//      locally using an AI quality score.
//      keyword_hash = BLAKE3(keyword + salt), salt generated randomly per
//      query to prevent cross-query correlation.
//
// Bias Contrast View:
//      Compares Shadow Index results against a conventional search snapshot
//      to highlight ranking differences and potential algorithmic bias.
//      Differences are rendered as a Mermaid diff diagram.
// ─────────────────────────────────────────────────────────────────────────────
...
// ── P2P keyword anonymization (BLAKE3 — v0.10.0) ───────────────────────────────
// [v0.10.0] BLAKE3 is now the canonical hash throughout Diatom (blocker, wasm_sandbox,
// PIR query indistinguishability, freeze integrity). anonymize_keyword() was already
// using blake3::hash — no change needed beyond ensuring the same crate version is used.
...
pub fn estimate_lean(domain: &str) -> PoliticalLean {
 ...
 // No P2P connection; fall back to local results in the UI.
...
mod tests {
    ...
    fn tfidf_basic_search() {
        ...
        // Rust docs should score higher than Python doc
