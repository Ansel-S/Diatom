// ─────────────────────────────────────────────────────────────────────────────
// diatom/src-tauri/src/wifi_trust.rs  — v0.12.0  [F-05]
//
// Wi-Fi Trust Scanner — 公共 Wi-Fi 自动保护
//
// Detects when Diatom is running on an open (unencrypted) Wi-Fi network and
// automatically activates GhostPipe DoH tunneling for all Diatom-internal
// requests. Also upgrades all HTTP navigations to HTTPS when on an untrusted
// network. Users can mark networks as trusted by SSID + BSSID pair.
//
// Platform detection:
//   macOS:   CWWiFiClient via `networksetup` / `/System/Library/PrivateFrameworks`
//   Windows: `netsh wlan show interfaces` subprocess
//   Linux:   /proc/net/wireless + `nmcli device wifi`
//
// Risk: SSID spoofing — a malicious AP can clone a trusted SSID name.
// Mitigation: trust is keyed on (SSID, BSSID) pair, not SSID alone.
// ─────────────────────────────────────────────────────────────────────────────
...
fn macos_wifi() -> Option<WifiInfo> {
    // `networksetup -getairportnetwork en0` → "Current Wi-Fi Network: NetworkName"
    ...
    // `airport -I` gives security type and BSSID
...
fn linux_wifi() -> Option<WifiInfo> {
    // Use nmcli for reliable output
    let out = std::process::Command::new("nmcli")
        .args(["-t", "-f", "ACTIVE,SSID,BSSID,SECURITY", "device", "wifi"])
        .output().ok()?;
    let text = String::from_utf8_lossy(&out.stdout);
    ...
    for line in text.lines() {
        ...
        if parts.len() >= 4 && parts[0] == "yes" {
            ...
            let bssid    = parts[2].trim().to_owned();
            let sec_text = parts[3..].join(":").to_lowercase();
...
pub fn is_trusted(db: &crate::db::Db, ssid: &str, bssid: &str) -> bool {
    trusted_networks(db).iter().any(|n| n.ssid == ssid && n.bssid == bssid)
}

/// Add a network to the trusted list.
pub fn trust_network(db: &crate::db::Db, ssid: &str, bssid: &str) -> Result<()> {
...
pub fn distrust_network(db: &crate::db::Db, ssid: &str, bssid: &str) -> Result<()> {
...
fn trusted_networks(db: &crate::db::Db) -> Vec<TrustedNetwork> {
    db.get_setting("wifi_trusted_networks")
        .and_then(|j| serde_json::from_str(&j).ok())
        .unwrap_or_default()
}

fn persist_trusted_networks(db: &crate::db::Db, networks: &[TrustedNetwork]) -> Result<()> {
