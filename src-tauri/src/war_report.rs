...
impl WarReport {
    pub fn from_row(row: &WarReportRow) -> Self {
        ...
        WarReport {
            ...
            summary_headline: summary_headline(row.tracking_block_count, row.fingerprint_noise_count),
        }
    }
}
...
fn block_narrative(n: i64) -> String {
    match n {
        0        => "本周追踪器安静得出奇。或许他们已经放弃了？".to_owned(),
        1..=99   => format!("Diatom 在本周拦截了 {n} 次监控探针，令其在到达渲染器之前就已熄灭。"),
        100..=999 => format!("本周共有 {n} 次追踪请求被截断于协议层。每一次都是你未曾经历过的数据收割。"),
        1000..=9999 => format!("{n} 次。那是 {n} 次有人试图为你建立档案。Diatom 让每一次尝试都徒劳无功。"),
...
fn noise_narrative(n: i64) -> String {
    match n {
        ...
        1000..=99_999 => format!("{n} 次随机噪声注入。每次都意味着一份错误的 Canvas 指纹、一个虚假的 WebGL 签名。追踪者收获的，是一座幻影城市。"),
...
fn ram_narrative(mb: f64) -> String {
    ...
    if mb < 100.0 {
        return format!("Deep Sleep 为你的设备回收了约 {mb:.0} MB RAM。\
            这些内存原本会因标签僵尸而永久燃烧。");
    }
    if mb < 500.0 {
        return format!("约 {mb:.0} MB 内存在本周得以复活。\
            {browser_name} 会让这些内存静静发热直至你关闭浏览器。Diatom 不会。",
            browser_name = "某些浏览器");
    }
    format!("{mb:.0} MB RAM——本周从标签地狱中被解救。\
...
fn time_narrative(min: f64) -> String {
    ...
    if min < 60.0 {
        return format!("本周约 {min:.0} 分钟未被低质量内容消耗。\
            这些时间是你的。去做任何值得做的事。");
    }
    let hrs = min / 60.0;
    format!("{hrs:.1} 小时。那是你本周因 Diatom 过滤而未曾浪费的时间。\
...
mod tests {
    ...
    fn zero_report_graceful() {
        let row = WarReportRow {
            tracking_block_count: 0, fingerprint_noise_count: 0,
            ram_saved_mb: 0.0, time_saved_min: 0.0,
        };
        let r = WarReport::from_row(&row);
        assert!(!r.block_narrative.is_empty());
    }
