...
pub async fn cmd_nostr_fetch(
    ...
    let key    = *state.master_key.lock().unwrap();
    let relay  = nostr_relay(&state);
    let events = crate::sync::nostr::fetch(&relay, kind, &key).await.map_err(es)?;
