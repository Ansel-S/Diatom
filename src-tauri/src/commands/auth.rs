...
pub async fn cmd_totp_add(
    account: crate::auth::totp::TotpAccount, state: St<'_>,
...
pub async fn cmd_totp_import(
    format: String, data: String, state: St<'_>,
) -> Result<u32, String> {
    state.with_master_key(|key| {
        state.totp.lock().unwrap().import(&format, &data, &state.db, key).map_err(es)
    })
}
...
pub async fn cmd_trust_set(
    domain: String, level: String, state: St<'_>,
) -> Result<(), String> {
