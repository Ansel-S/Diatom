// ─────────────────────────────────────────────────────────────────────────────
// diatom/src-tauri/src/totp.rs  — v0.9.5
//
...
// [NEW-v0.9.5] Steam Guard TOTP (26-char alphabet, base64/base32 secret).
// [NEW-v0.9.5] next_code field — current + next-window code in TotpCode.
// [NEW-v0.9.5] Import: Aegis JSON, Bitwarden JSON, URI list, 2FAS JSON.
// [NEW-v0.9.5] Export: Aegis-compatible unencrypted JSON.
// [NEW-v0.9.5] SHA-256/512 algorithm support alongside SHA-1.
// ─────────────────────────────────────────────────────────────────────────────
...
impl TotpStore {
        ...
        for raw in db.totp_list_raw().unwrap_or_default() {
            match decrypt_secret(&raw.secret_enc, master_key) {
                Ok(secret) => {
                    ...
                    let algorithm = raw.algorithm.as_deref()
                        .map(TotpAlgorithm::from_str).unwrap_or_default();
                    ...
                    entries.insert(raw.id.clone(), TotpEntry {
                        ...
                        secret, algorithm, digits, period, domains, added_at: raw.added_at,
                    });
                }
    ...
    pub fn add(
        &mut self,
        issuer: &str, account: &str, secret: &str,
        algorithm: TotpAlgorithm, digits: u8, period: u32,
        ...
        db.totp_insert_v2(&id, issuer, account, &enc, &dj, now,
                          algorithm.as_str(), digits, period)?;
        let entry = TotpEntry {
            id: id.clone(), issuer: issuer.to_owned(), account: account.to_owned(),
            secret: clean, algorithm, digits, period, domains, added_at: now,
        };
    ...
    pub fn add_from_uri(
        &mut self, uri: &str, domains: Vec<String>,
        db: &crate::db::Db, master_key: &[u8; 32],
    ) -> Result<TotpEntry> {
        let p = parse_otpauth_uri(uri)?;
        self.add(
            &p.issuer.unwrap_or_default(),
            &p.account.unwrap_or_default(),
            &p.secret, p.algorithm, p.digits, p.period,
            domains, db, master_key,
        )
    }
    ...
    pub fn generate(&self, id: &str) -> Result<TotpCode> {
        ...
        let now     = crate::db::unix_now() as u64;
        ...
        Ok(TotpCode {
            entry_id: id.to_owned(), issuer: e.issuer.clone(), account: e.account.clone(),
            code, next_code, valid_until, period: e.period,
        })
    }
    ...
    pub fn import_aegis(
        &mut self, json: &str, db: &crate::db::Db, master_key: &[u8; 32],
    ) -> Result<usize> {
        #[derive(Deserialize)]
        struct AInfo { secret: String, algo: String, digits: u8, period: u32 }
        #[derive(Deserialize)]
        struct AEntry {
            #[serde(rename = "type")] kind: String,
            issuer: Option<String>, name: Option<String>, info: AInfo,
        }
        #[derive(Deserialize)]
        struct ARoot { entries: Vec<AEntry> }

        let root: ARoot = serde_json::from_str(json).context("Aegis JSON")?;
        let mut n = 0usize;
        for e in root.entries {
            let algo = if e.kind.to_lowercase() == "steam" { TotpAlgorithm::Steam }
                       else { TotpAlgorithm::from_str(&e.info.algo) };
            if self.add(
                e.issuer.as_deref().unwrap_or(""),
                e.name.as_deref().unwrap_or(""),
                &e.info.secret, algo, e.info.digits, e.info.period,
                vec![], db, master_key,
            ).is_ok() { n += 1; }
        }
        Ok(n)
    ...
    pub fn import_bitwarden_json(
        ...
        #[derive(Deserialize)]
        struct BwLogin { #[serde(default)] totp: Option<String>,
                         #[serde(default)] username: Option<String> }
        #[derive(Deserialize)]
        struct BwItem { name: String, #[serde(default, rename = "type")] kind: u8,
                        login: Option<BwLogin> }
        #[derive(Deserialize)]
        struct BwRoot { items: Vec<BwItem> }
                ...
                if let Some(totp) = login.totp.filter(|t| !t.is_empty()) {
                    let ok = if totp.starts_with("otpauth://") {
                        ...
                        self.add(&item.name,
                            ...
                            &totp, TotpAlgorithm::Sha1, 6, 30,
                            vec![], db, master_key).is_ok()
                    };
                    if ok { n += 1; }
                }
            }
    ...
    pub fn import_uri_list(
        &mut self, text: &str, db: &crate::db::Db, master_key: &[u8; 32],
    ) -> Result<usize> {
        let mut n = 0usize;
        for line in text.lines() {
            let line = line.trim();
            if line.starts_with("otpauth://") || line.starts_with("steam://") {
                if self.add_from_uri(line, vec![], db, master_key).is_ok() { n += 1; }
            }
        }
        Ok(n)
    }
    ...
    pub fn import_2fas(
        ...
        #[derive(Deserialize)]
        struct TfOtp {
            account: Option<String>, issuer: Option<String>, secret: String,
            #[serde(rename = "tokenType")] token_type: Option<String>,
            algorithm: Option<String>, digits: Option<u8>, period: Option<u32>,
        }
        #[derive(Deserialize)]
        struct TfSvc { otp: TfOtp }
        #[derive(Deserialize)]
        struct TfRoot { services: Vec<TfSvc> }

        let root: TfRoot = serde_json::from_str(json).context("2FAS JSON")?;
        let mut n = 0usize;
        for svc in root.services {
            let otp = svc.otp;
            let algo = if otp.token_type.as_deref() == Some("STEAM") { TotpAlgorithm::Steam }
                       else { TotpAlgorithm::from_str(otp.algorithm.as_deref().unwrap_or("SHA1")) };
            if self.add(
                otp.issuer.as_deref().unwrap_or(""),
                otp.account.as_deref().unwrap_or(""),
                &otp.secret, algo,
                otp.digits.unwrap_or(6), otp.period.unwrap_or(30),
                vec![], db, master_key,
            ).is_ok() { n += 1; }
        }
        Ok(n)
    ...
    pub fn export_aegis_json(&self) -> Result<String> {
        #[derive(Serialize)]
        struct ExInfo { secret: String, algo: String, digits: u8, period: u32 }
        #[derive(Serialize)]
        struct ExEntry {
            #[serde(rename = "type")] kind: &'static str,
            issuer: String, name: String, info: ExInfo,
        }
        #[derive(Serialize)]
        struct ExRoot { version: u8, entries: Vec<ExEntry> }

        let entries: Vec<ExEntry> = self.entries.values().map(|e| ExEntry {
            kind: if e.algorithm == TotpAlgorithm::Steam { "steam" } else { "totp" },
        ...
        serde_json::to_string_pretty(&ExRoot { version: 1, entries })
...
pub fn parse_otpauth_uri(uri: &str) -> Result<ParsedOtpUri> {
    ...
    if uri.starts_with("steam://") {
        return Ok(ParsedOtpUri {
            issuer: Some("Steam".to_owned()), account: None,
            secret: uri.trim_start_matches("steam://").to_owned(),
            algorithm: TotpAlgorithm::Steam, digits: 5, period: 30,
        });
    }

    let parsed = Url::parse(uri).context("parse otpauth URI")?;
    if parsed.scheme() != "otpauth" { bail!("not an otpauth URI"); }

    let label_raw = parsed.path().trim_start_matches('/');
    let label_dec = urlencoding::decode(label_raw)
        .map(|c| c.into_owned()).unwrap_or_else(|_| label_raw.to_owned());
    let (path_issuer, account) = if let Some(pos) = label_dec.find(':') {
        ...
        (None, Some(label_dec.trim().to_owned()).filter(|s| !s.is_empty()))
    };
    ...
    if secret.is_empty() { bail!("otpauth URI missing secret"); }

    // Detect Steam from issuer query
...
pub fn generate_code(secret: &str, algorithm: TotpAlgorithm, digits: u8, counter: u64) -> Result<String> {
    if algorithm == TotpAlgorithm::Steam {
...
fn steam_code(secret: &str, counter: u64) -> Result<String> {
    let key = base32::decode(Alphabet::Rfc4648 { padding: false }, secret)
        ...
        .or_else(|| {
            use base64::Engine;
            base64::engine::general_purpose::STANDARD.decode(secret).ok()
                .or_else(|| base64::engine::general_purpose::STANDARD_NO_PAD.decode(secret).ok())
        })
    ...
    for _ in 0..5 { code.push(STEAM_CHARS[n as usize % 26]); n /= 26; }
    Ok(code)
}
...
fn hotp_sha1_trunc(key: &[u8], counter: u64) -> Result<u32> {
    let raw = hotp_sha1_raw(key, counter)?;
    let offset = (raw[19] & 0xf) as usize;
    Ok(u32::from_be_bytes([raw[offset] & 0x7f, raw[offset+1], raw[offset+2], raw[offset+3]]))
}
...
fn hotp_sha256(key: &[u8], counter: u64) -> Result<u32> {
    let mut mac = HmacSha256::new_from_slice(key).map_err(|_| anyhow::anyhow!("HMAC-SHA256 key"))?;
    mac.update(&counter.to_be_bytes());
    let r = mac.finalize().into_bytes();
    let o = (r[31] & 0xf) as usize;
    Ok(u32::from_be_bytes([r[o] & 0x7f, r[o+1], r[o+2], r[o+3]]))
}

fn hotp_sha512(key: &[u8], counter: u64) -> Result<u32> {
    let mut mac = HmacSha512::new_from_slice(key).map_err(|_| anyhow::anyhow!("HMAC-SHA512 key"))?;
    mac.update(&counter.to_be_bytes());
    let r = mac.finalize().into_bytes();
...
mod tests {
    ...
    fn steam_code_is_5_chars() {
        let key = base32::encode(Alphabet::Rfc4648 { padding: false }, b"steamguardkey1234567");
    ...
    fn algorithm_roundtrip() {
        for a in [TotpAlgorithm::Sha1, TotpAlgorithm::Sha256, TotpAlgorithm::Sha512, TotpAlgorithm::Steam] {
