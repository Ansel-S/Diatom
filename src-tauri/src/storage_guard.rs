...
pub fn report(db: &crate::db::Db, budget: &StorageBudget) -> StorageReport {
    ...
    let (museum_bytes, bundle_count, oldest_ts): (i64, u32, Option<i64>) = conn.query_row(
        "SELECT COALESCE(SUM(bundle_size),0), COUNT(*), MIN(frozen_at) FROM museum_bundles",
        [],
        |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?)),
    ).unwrap_or((0, 0, None));

    let kpack_bytes: i64 = conn.query_row(
...
pub fn evict_lru(
    ...
    let conn = db.0.lock().unwrap();
    let target_bytes = (budget.museum_budget_mb as f64 * target_pct as f64 / 100.0 * 1_048_576.0) as i64;
    ...
    let bundles: Vec<(String, String, i64)> = stmt.query_map([], |r| {
        Ok((r.get(0)?, r.get(1)?, r.get(2)?))
    })?.filter_map(|r| r.ok()).collect();

    // Current total
    let total: i64 = conn.query_row(
        "SELECT COALESCE(SUM(bundle_size),0) FROM museum_bundles", [], |r| r.get(0)
    ).unwrap_or(0);
...
mod tests {
    ...
    fn default_budget_is_reasonable() {
        ...
        assert!(b.kpack_budget_mb  >= 2048, "at least 2 GB for knowledge packs");
