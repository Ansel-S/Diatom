// ─────────────────────────────────────────────────────────────────────────────
// diatom/src-tauri/src/panic_button.rs  — v0.12.0  [F-01]
//
// Panic Button — 紧急隐私锁
//
// A single global hotkey (Cmd/Ctrl+Shift+.) instantly hides all Tauri windows
// and replaces the active tab title/favicon with a configurable decoy page.
// On restoration (second keypress or tray icon), the full workspace is intact.
//
// Integration:
//   • Registered via tauri_plugin_global_shortcut at startup when lab is ON.
//   • Decoy URL uses the existing decoy.rs infrastructure.
//   • Optional "wipe mode" integrates with workspace::fire_workspace().
//   • Frontend: panic-overlay.js injects a full-viewport decoy DOM over WebView.
//   • No network requests during panic — all local DOM manipulation.
// ─────────────────────────────────────────────────────────────────────────────
...
pub fn activate(app: &AppHandle, config: &PanicConfig) {
    ...
    // Emit to frontend — panic-overlay.js will inject the decoy DOM
...
pub fn load_config(db: &crate::db::Db) -> PanicConfig {
...
pub fn save_config(db: &crate::db::Db, cfg: &PanicConfig) -> anyhow::Result<()> {
