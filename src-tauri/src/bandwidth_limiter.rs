// ─────────────────────────────────────────────────────────────────────────────
// diatom/src-tauri/src/bandwidth_limiter.rs  — v0.12.0  [F-12]
//
// Network Bandwidth Limiter — 带宽分配控制
//
// Extends NetMonitor with per-domain and global bandwidth rate limiting using
// a token-bucket algorithm. Useful for metered connections, background tab
// throttling, and preventing a single media-heavy tab from saturating the
// connection.
//
// Token bucket algorithm:
//   Each domain bucket has capacity C bytes and refills at R bytes/sec.
//   On each outbound request, consume(est_bytes) is called. If the bucket is
//   depleted, the request is queued with a calculated delay.
//
// Exemptions:
//   Diatom-internal requests (filter list updates, Sentinel) are always exempt.
//   Large file downloads can be given a per-request "unlimited" bypass.
//
// Integration:
//   The existing blocker.rs intercept path calls BandwidthLimiter::check()
//   before forwarding requests. Results are surfaced in bandwidth-panel.js.
// ─────────────────────────────────────────────────────────────────────────────
...
impl TokenBucket {
    ...
    fn consume(&mut self, bytes: u64) -> Duration {
        ...
        if self.tokens >= bytes as f64 {
            ...
            self.tokens = 0.0;
            // Calculate how long to wait for the bucket to refill enough
...
impl BandwidthLimiter {
    ...
    pub fn check(&self, domain: &str, estimated_bytes: u64) -> Duration {
        // Internal Diatom requests are always exempt
    ...
    pub fn set_global_limit(&self, kbps: u32) {
        *self.global_limit_kbps.lock().unwrap() = kbps;
        // Reset the global bucket so it's rebuilt with the new rate
    ...
    pub fn upsert_rule(&self, rule: BandwidthRule) {
        ...
        // Invalidate the bucket so it's rebuilt with the new rate
    ...
    /// Load rules from DB.
    pub fn load_from_db(&self, db: &crate::db::Db) {
    ...
    /// Persist rules to DB.
    pub fn save_to_db(&self, db: &crate::db::Db) -> Result<()> {
        let rules = self.rules.lock().unwrap();
