// ─────────────────────────────────────────────────────────────────────────────
// diatom/src-tauri/src/museum_version.rs
//
// Museum Version control — Git -style snapshot diffing
//
// User need: "I want to compare the March version of this page against January."
//
// Design:
//   Each time freeze() is called for a URL, if the URL already has a history the old
//   version is appended rather than overwritten.
//   Each version stores::
//     - version_id: UUID
//     - museum_id: the corresponding Museum entry
//     - url: raw URL
//     - frozen_at: Unix timestamp
//     - content_hash: BLAKE3 hash (for change detection)
//     - diff_from_prev: Myers diff against the previous version (line-level, up to 64 KB)
//     - size_bytes: rawraw content size
//
// diffcompute:
//   - Compute Myers diff (line-level) between two HTML version snapshots.
// - Identical content is not stored as a new version (content_hash deduplication).
// - Frontend request twoversionversions diff
//
// Temporal Integrity Auditor:
// When the user visits a URL with Museum history, computes a diff between the current
// online content and the most recent snapshot.
// If the diff exceeds a threshold (default: 5%), injects a "⚠ content changed" banner.
// ─────────────────────────────────────────────────────────────────────────────
...
pub fn compute_diff(old_text: &str, new_text: &str) -> String {
 ...
 // Myers diff: LCS-based line diff
    ...
    while oi < old_lines.len() || ni < new_lines.len() {
        ...
        // cap diff at 64KB to avoid huge storage
        if result.len() > 65536 {
