// ─────────────────────────────────────────────────────────────────────────────
// diatom/src-tauri/src/tab_proxy.rs  — v0.12.0  [F-03]
//
// Per-Tab Independent Proxy — 独立代理 IP
//
// Each tab can be assigned an independent SOCKS5 or HTTP proxy, enabling true
// IP isolation between browsing contexts without requiring separate workspaces.
//
// Architecture:
//   Tauri 2.0 sets proxy at WebviewWindow creation time via
//   WebviewWindowBuilder::proxy(). Because Diatom uses a single WebviewWindow,
//   per-tab proxy requires a PAC (Proxy Auto-Config) script injected via
//   initialization_script that routes WebView fetch/XHR calls through a loopback
//   Diatom proxy dispatcher (CONNECT tunnel on 127.0.0.1:3182x per tab slot).
//   The Rust dispatcher selects the upstream proxy per connection based on a
//   tab-ID cookie injected into request headers.
//
// Risk: MEDIUM — users who set incorrect proxy addresses lose connectivity
// for that tab only. The loopback CONNECT tunnel requires OS socket permissions.
// ─────────────────────────────────────────────────────────────────────────────
...
impl ProxyConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        // Reject non-HTTPS SearXNG-style SSRF vectors
    ...
    pub fn generate_pac_script(&self, tab_proxies: &[(String, ProxyConfig)]) -> String {
        ...
        format!(
            "function FindProxyForURL(url, host) {{\n\
             // Tab-aware routing via X-Diatom-Tab-Id request header\n\
             var myHostHeader = '';\n\
             {cases}\
             return 'DIRECT';\n\
             }}"
...
pub fn save_proxy(db: &crate::db::Db, tab_id: &str, proxy: &ProxyConfig) -> Result<()> {
...
pub fn load_proxy(db: &crate::db::Db, tab_id: &str) -> Option<ProxyConfig> {
    let key = format!("tab_proxy:{}", tab_id);
    db.get_setting(&key)
