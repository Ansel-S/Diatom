// ─────────────────────────────────────────────────────────────────────────────
// diatom/src-tauri/src/search_engine.rs  — v0.12.0  [F-10]
//
// Privacy Search Integration — 零追踪搜索引擎
//
// Expands Diatom's search engine options with privacy-first alternatives:
//   • Brave Search   — zero-tracking, independent index
//   • SearXNG        — self-hosted metasearch (user-configurable endpoint)
//   • Kagi           — paid, no ads, no tracking (requires API key in Vault)
//
// All queries are routed through GhostPipe if enabled, preventing the search
// engine from correlating queries to a static IP address.
//
// Security: SearXNG endpoint validated as HTTPS-only to prevent SSRF.
// ─────────────────────────────────────────────────────────────────────────────
...
pub fn builtin_engines() -> Vec<SearchEngine> {
    vec![
        ...
        SearchEngine {
            ...
            privacy_tier: PrivacyTier::Independent,
            requires_key: false,
            description: "No personal data stored. Uses Bing for some results.",
        },
        SearchEngine {
            id: "searxng",
            name: "SearXNG",
...
pub fn get_default(db: &crate::db::Db) -> String {
    db.get_setting("default_search_engine")
        .unwrap_or_else(|| "brave".to_owned())
}

/// Set the default search engine. Validates the ID exists.
pub fn set_default(db: &crate::db::Db, engine_id: &str) -> Result<()> {
    // Allow custom SearXNG endpoint override with validation
...
pub fn set_searxng_endpoint(db: &crate::db::Db, endpoint: &str) -> Result<()> {
    validate_searxng_endpoint(endpoint)?;
