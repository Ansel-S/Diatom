...
pub async fn check_email(
    ...
    if status == 401 {
        anyhow::bail!("HIBP email lookup requires a paid API key (401 Unauthorized)");
    }
    if status == 429 {
        anyhow::bail!("HIBP rate limit reached — retry after 1 minute (429 Too Many Requests)");
    }
    if !resp.status().is_success() {
        anyhow::bail!("HIBP email request failed with HTTP {}", status);
    }
...
pub fn cache_password_result(db: &crate::storage::db::Db, password_sha1: &str, result: &PasswordBreachResult) {
    if let Ok(json) = serde_json::to_string(result) {
        let key = format!("breach_pw:{}", &password_sha1[..16]);
        let expiry = crate::storage::db::unix_now() + CACHE_TTL_SECS;
...
pub fn load_cached_password(db: &crate::storage::db::Db, password_sha1: &str) -> Option<PasswordBreachResult> {
    let key = format!("breach_pw:{}", &password_sha1[..16]);
    let entry = db.get_setting(&key)?;
    let colon = entry.find(':')?;
    let expiry: i64 = entry[..colon].parse().ok()?;
    if crate::storage::db::unix_now() > expiry {
        return None; // Expired
    }
...
///
/// [FIX-B08] The previous implementation used `unix_now() / 3600 % UAS.len()`
/// which produced the identical UA for every check within the same clock hour
/// and leaked the hour boundary in traffic timing. Replaced with a
/// thread_rng-based random pick so each call independently selects a UA.
