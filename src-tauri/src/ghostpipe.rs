// ─────────────────────────────────────────────────────────────────────────────
// diatom/src-tauri/src/ghostpipe.rs
//
// GhostPipe — DNS Tunnel (browser-native edition)
//
// Note: a full standalone app (global TUN/TAP takeover) is out of scope for this release.
// This module implements the "browser-native driver" portion: tunnels only Diatom's own HTTP requests.
//
// How it works::
//   Routes Diatom outbound HTTP requests (filter list fetches, Sentinel queries, etc.)
//   via DoH (DNS over HTTPS), disguised as regular DNS queries.
//   Prevents network administrators from identifying Diatom's background requests via HTTP traffic analysis.
//
// Limitations (browser-native version):
//   - Protects only Diatom processDiatom's ownrequest
// - Does not tunnel page content (page requests go via normal HTTPS).
// - Does not provide global VPN-style routing (TUN/TAP requires a standalone app).
//
// Implementation:
// reqwest Configuration via Cloudflare DoH :
//   https://cloudflare-dns.com/dns-query
// - "Packet Fragmentation" mode: fragments requests via DoH to further obscure traffic patterns.
// ─────────────────────────────────────────────────────────────────────────────
...
pub async fn resolve_via_doh(domain: &str, endpoint: &str) -> Result<Vec<std::net::IpAddr>> {
    ...
    let addrs = resp.answer.unwrap_or_default()
        .into_iter()
...
pub async fn resolve_fragmented(domain: &str, endpoints: &[String]) -> Result<Vec<std::net::IpAddr>> {
    ...
    let futures: Vec<_> = endpoints.iter().map(|ep| {
        let domain = domain.to_owned();
        let ep = ep.clone();
        async move { resolve_via_doh(&domain, &ep).await }
    }).collect();

    // Race all futures, return first success
...
impl GhostPipeStatus {
    pub fn from_config(cfg: &GhostPipeConfig, requests_tunneled: u64) -> Self {
        Self {
            enabled: cfg.enabled,
            mode: if cfg.diatom_own_only { "browser-integrated" } else { "system-wide" },
...
mod tests {
    ...
    fn default_config_is_disabled() {
        ...
        assert!(cfg.diatom_own_only, "Must default to browser-integrated only");
    }
